...
const ChipComponent: React.FC = () => {
  ...
  const items: string[] = ["John Doe", "Jane Smith", "Nick Giannopoulos", "Alice Johnson"];
    ...
    <div className="chip-container">
      <div className="chips">
        {chips.map(chip => (
          <div key={chip.id} className="chip">
            {chip.label}
            <button className="remove" onClick={() => handleChipRemove(chip.id)}>
              X
            </button>
          </div>
        ))}
      ...
      <ul className="item-list">
        {filteredItems.map(item => (
          <li key={item} onClick={() => handleChipClick(item)}>{item}</li>
        ))}
      </ul>
    </div>
  );
